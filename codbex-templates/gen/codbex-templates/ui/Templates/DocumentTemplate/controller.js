angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', 'messageHub', 'entityApi', 'Extensions', function ($scope, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-templates-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "Templates" && e.view === "DocumentTemplate" && (e.type === "page" || e.type === undefined));
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("DocumentTemplate", `Unable to count DocumentTemplate: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				request.then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("DocumentTemplate", `Unable to list/filter DocumentTemplate: '${response.message}'`);
						return;
					}
		...
		$scope.deleteEntity = function (entity) {
			...
			messageHub.showDialogAsync(
				...
				`Are you sure you want to delete DocumentTemplate? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("DocumentTemplate", `Unable to delete DocumentTemplate: '${response.message}'`);
							return;
						}
						$scope.loadPage($scope.dataPage, $scope.filter);

...
const PURCHASE_INVOICE_ID = 12;

response.setContentType("text/html");

let connection = database.getConnection("DefaultDB");
try {
    let statement = connection.prepareStatement(`SELECT * FROM "CODBEX_DOCUMENTTEMPLATE" WHERE "DOCUMENTTEMPLATE_TYPE" = ${PURCHASE_INVOICE_ID}`);
